...
namespace Meteor.Database.Dapper.Operations.Db
{
    public static class Extensions
    {
        ...
        public static QueryPage<TOutput> CreateQueryPage<TInput, TOutput>(this DbQueryPageAsync<TInput, TOutput> dbOperation, IEnumerable<TOutput> items,
            long totalCount) where TInput : Database.Dapper.Operations.Db.IQueryPageInput
        {
        ...
        public static async Task<QueryPage<TOutput>> SelectQueryPageAsync<TInput, TOutput>(this DbQueryPageAsync<TInput, TOutput> dbOperation,
            ISqlDialect selectItems, ISqlDialect selectCount) where TInput : Database.Dapper.Operations.Db.IQueryPageInput
        {
        ...
        public static async Task<QueryPage<TOutput>> SelectQueryPageAsync<TInput, TOutput>(this DbQueryPageAsync<TInput, TOutput> dbOperation,
            ISqlFactory sqlFactory, string tableName) where TInput : Database.Dapper.Operations.Db.IQueryPageInput
        {
            var items = await dbOperation.LazyDbConnection
